...
function brandedErrorResponse(request: Request, error?: unknown): Response {
...
async function normalizeCatastrophicSsrResponse(request: Request, response: Response): Promise<Response> {
  ...
  console.error(error);
  return brandedErrorResponse(request, error);
}
...
export default {
  async fetch(request: Request, env: unknown, ctx: unknown) {
    try {
      ...
      console.error("[server.ts] Fatal error:", error);
      return brandedErrorResponse(request, error);
    }
  },
};
